<template>
  <div class="hello">
    ...
    <div class="hell_top">
      <img
        src="../assets/t2.png"
        style="width: 20px; height: 18px; position: relative; left: -20px"
        @click="add"
      />
      <span style="font-size: 17px; font-weight: 900" @click="requestFullScreen"
        >我的足迹记录</span
      >
      <img
        src="../assets/t1.png"
        style="width: 22px; height: 30px; position: relative; left: 25px"
        @click="shanchu"
      />
    </div>
    <div class="top_t">
    ...
    <div class="top_three" @click="showNum = true">
      <div>
        <span
          >累计导航 <span class="three_text">{{ Num01 }}</span> 公里
      ...
      <div v-for="(v, i) in ArrData" :key="i" style="margin-bottom: 15px;">
        ...
        <div class="hezi">
          <div
            ...
            :key="index"
            @click="handlerIndex(item, index, i)"
          >
            <div style="width: 100%" class="hezi_top">
              <img
                v-if="item.walk"
                src="../assets/t5.png"
                style="width: 51px; height: 20px; margin-right: 5px"
              />
              <img
                v-else
                src="../assets/t6.png"
              ...
              />
              <span>
                {{ item.km >= 1000 ? (item.km * 0.001).toFixed(1) : item.km
                }}{{ item.km >= 1000 ? "公里" : "米" }}</span
              >
              <img
                v-if="item.cbp"
                src="../assets/t10.png"
    ...
    >
      ...
      />
      <van-field v-model="from.lutu1" label="地址1" placeholder="请输入地址1" />
      <van-field v-model="from.lutu2" label="地址2" placeholder="请输入地址2" />
      <van-button style="width: 100%" type="danger" @click="del()"
        >删除</van-button
      >
    </van-popup>
    <!-- top日期月份 -->
    <van-popup v-model:show="showrq" position="bottom">
      <van-date-picker
        ...
        @cancel="showrq = false"
      />
      <van-button style="width: 100%" type="primary" @click="handleadd()"
        >添加行程</van-button
      >
    </van-popup>
    <!-- top公里数轨迹 -->
